...
syscall(uint32_t syscallno, uint32_t a1, uint32_t a2, uint32_t a3, uint32_t a4, uint32_t a5) {
    ...
    switch (syscallno) {
        case SYS_cputs:
            cprintf("systemcall is SYS_cputs\n");
            sys_cputs((char*)a1, (size_t)a2);
            ret = 0;
            break;
        case SYS_cgetc:
            cprintf("systemcall is SYS_cgetc\n");
            ret = sys_cgetc();
            break;
        case SYS_getenvid:
            cprintf("systemcall is SYS_getenvid\n");
            ret = sys_getenvid();
            break;
        case SYS_env_destroy:
            cprintf("systemcall is SYS_env_destroy\n");
            ret = sys_env_destroy((envid_t)a1);
            break;
        case SYS_yield:
            cprintf("systemcall is SYS_yield\n");
            sys_yield();
        case SYS_exofork:
            cprintf("systemcall is SYS_exofork\n");
            return sys_exofork();
        case SYS_env_set_status:
            cprintf("systemcall is SYS_env_set_status\n");
            return sys_env_set_status(a1, a2);
        case SYS_page_alloc:
            cprintf("systemcall is SYS_page_alloc\n");
            return sys_page_alloc(a1, (void*)a2, a3);
        case SYS_page_map:
            cprintf("systemcall is SYS_page_map\n");
            return sys_page_map(a1, (void*)a2, a3, (void*)a4, a5);
        case SYS_page_unmap:
            cprintf("systemcall is SYS_page_unmap\n");
