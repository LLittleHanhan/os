...
void mem_init(void) {
    ...
    cprintf("alloc 1 page for kern_pgdir\n\n");
    ...
    // Your code goes here:
    cprintf("struct PageInfo size is %dB\n", sizeof(struct PageInfo));
    cprintf("alloc %d pages for pages\n\n", sizeof(struct PageInfo) * npages / 4096);
    pages = (struct PageInfo*)boot_alloc(sizeof(struct PageInfo) * npages);
    memset(pages, 0, sizeof(struct PageInfo) * npages);
    //////////////////////////////////////////////////////////////////////
    // Make 'envs' point to an array of size 'NENV' of 'struct Env'.
    // LAB 3: Your code here.
    cprintf("struct Env size is %dB\n", sizeof(struct Env));
    cprintf("alloc %d pages for envs\n\n", sizeof(struct Env) * NENV / 4096);
    ...
    // or page_insert
    page_init();
    cprintf("\ncheck_page_free_list bgein\n");
    check_page_free_list(1);
    cprintf("\ncheck_page_alloc bgein\n");
    check_page_alloc();
    cprintf("\ncheck_page bgein\n");
    check_page();
    cprintf("\nbegin boot map region\n");
    //////////////////////////////////////////////////////////////////////
...
pte_t* pgdir_walk(pde_t* pgdir, const void* va, int create) {
    ...
    if (!(*pgdir_entry & PTE_P)) {
        if (create) {
            struct PageInfo* pgtable = page_alloc(1);
            if (pgtable == NULL) {
                cprintf("pgdir_walk return none because there is no free page to create page table\n");
                return NULL;
            }
            pgtable->pp_ref++;
            *pgdir_entry = page2pa(pgtable) | PTE_P | PTE_W | PTE_U;
        } else {
            cprintf("pgdir_walk return none because flag is 0\n");
            return NULL;
        }
    }
    ...
    for (offset = 0; offset < size; offset += PGSIZE, va += PGSIZE, pa += PGSIZE) {
        ...
        if (pgtable_entry != NULL) {
            *pgtable_entry = (pa | perm | PTE_P);
            ...
            cprintf("boot_map_region fails because pgdir_walk returns none\n");
...
int page_insert(pde_t* pgdir, struct PageInfo* pp, void* va, int perm) {
    ...
    if (pgtable_entry != NULL) {
        ...
        *pgtable_entry = page2pa(pp) | perm | PTE_P;
        ...
        cprintf("page_insert fails because pgdir_walk returns none\n");
        return -E_NO_MEM;
    }
    return 0;
}
...
page_lookup(pde_t* pgdir, void* va, pte_t** pte_store) {
    ...
    if (pgtable_entry == NULL || !(*pgtable_entry & PTE_P))
        return NULL;
    else {
        if (pte_store)
